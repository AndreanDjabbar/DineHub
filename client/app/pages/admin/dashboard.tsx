...
import { 
  FiLogOut, FiUserPlus, FiLayers, FiBookOpen
} from "react-icons/fi";  
...
// Import Types
import type { User, Table, MenuCategory, AddOn, AddOnOption } from "./components/types";

const AdminDashboard: React.FC = () => {
  const token = localStorage.getItem("token");
  const userString = localStorage.getItem("user");  
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState<'staff' | 'tables' | 'menu'>('staff');
  ...
  const [newAddOn, setNewAddOn] = useState<AddOn>({ name: '', minSelect: 0, maxSelect: 1, options: [] });
  const [newAddOnOption, setNewAddOnOption] = useState<AddOnOption>({ name: '', price: 0 });
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:4000/dinehub/api/menu/categories/${restaurantId}`, {
          method: "GET",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        });
        if(!response.ok){
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch("http://localhost:4000/dinehub/api/menu/categories", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({
  ...
  const handleAddMenuItem = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch("http://localhost:4000/dinehub/api/menu/items", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({
